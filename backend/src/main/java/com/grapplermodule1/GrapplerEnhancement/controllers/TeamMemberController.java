...
//    }
    ...
    public ResponseEntity<?> addNewMembers(@PathVariable("teamId") Long teamId, @RequestBody List<Long> userIds) {
        String debugUuid = UUID.randomUUID().toString();
        try {
            log.info("Add New Members API Called, UUID {}", debugUuid);
            List<TeamMembers> teamMembers = teamMembersService.addNewMembers(teamId, userIds);

            return new ResponseEntity<>(new CustomResponse<>(true, "New Members Added Successfully.", teamMembers), HttpStatus.OK);
        } catch (UserNotFoundException e) {
            log.error("UUID {} UserNotFoundException In Add New Members API, Exception {}", debugUuid, e.getMessage());
            return new ResponseEntity<>(new CustomResponse<>(false, e.getMessage(), null), HttpStatus.NOT_FOUND);
        } catch (TeamNotFoundException e) {
            log.error("UUID {} TeamNotFoundException In Add New Members API, Exception {}", debugUuid, e.getMessage());
            return new ResponseEntity<>(new CustomResponse<>(false, e.getMessage(), null), HttpStatus.NOT_FOUND);
        } catch (MemberAlreadyPresentException e) {
            log.error("UUID {} MemberAlreadyPresentException In Add New Members API, Exception {}", debugUuid, e.getMessage());
            return new ResponseEntity<>(new CustomResponseMessage(false, e.getMessage()), HttpStatus.CONFLICT);
        } catch (Exception e) {
            log.error("UUID {} Exception In Add New Members API, Exception {} ", debugUuid, e.getMessage());
            return new ResponseEntity<>(e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
